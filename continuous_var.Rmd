...
1. Calculate the quantile % for each of our sorted datapoint. Ex: The first data point would be 0% Quantile regardless of size of our data.
...
4. Draw a `y = x` line suggesting a hypothetical scenario where our data is normal and so we would observe the same quantiles.
...
Using our intuition, a right skewed data would mean we have outliers to end of our data. Implying we would observe deviations towards the end of distribution. Now if we remember how we plotted the quantiles, we know that the ourliers (the big values) would be given an x% quantile in the normal distribution which would be less than the outlier. Suggesting `y > x`. Hence we would observe data points to the left of normal line towards the end of the distribution in the Q-Q plot.
...
Using our intuition, a left skewed data would mean we have outliers at the beginning of our data. Implying the quantiles we observe at the beginning are smaller than same quantile points for normal distribution. Suggesting `y < x`. Hence we would observe data points to the right of the normal line at the start of the distribution in the Q-Q plot.
