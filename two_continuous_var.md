...
sample <- sample_n(movies,0.2*nrow(movies))
...
sample2 <- sample_n(movies,0.01*nrow(movies)) %>%
  filter(year > 2000)
...
```r
sample3 <- sample_n(movies, 0.01*nrow(movies)) #sample data
