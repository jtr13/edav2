...
2. [Install RStudio](https://r4ds.had.co.nz/introduction.html#rstudio){target="_blank"} (*r4ds*) -- Download the free, Desktop version for your OS. Working in this IDE will make working in R much more enjoyable. *As with R, stay current.* The latest version (as of `r Sys.Date()`) is `r rstudio_latest_version`. Click the **RStudio** menu, then "About RStudio" to see what version you have. (Note: RStudio, the company, is now Posit. RStudio, the product, is still RStudio.)
